...
public:
	...
	Vector operator+(const Vector& v) const;// 상수 벡터를 입력받아 값을 변화시키지 않고 Vector로 반환한다.
	Vector operator-(const Vector& v) const;// Vector& v는 실제 벡터의 복사본을 
											// 만드는 대신 전달된 벡터를 사용하는 것을
											// 의미합니다. 그렇기 때문에 참조된 Vector
											// 앞 const는 원본을 변화시키지 않겠다는 이야기입니다.


	Vector operator*(float s) const;	//벡터를 s만큼 빠르게 가속
	Vector operator/(float s) const;	//벡터를 s만큼 느리게 감속하는 함수
	
	Vector Nomalized()const;// 수정하지 않고, 정규화된 벡터를 반환합니다.
...
{ // ax*bx+ay*by 
	return a.x * b.x + a.y * b.y;// 3d라면 +a.z*b.z;
}
...
{// V = (Dx+Rx,Dy+Ry)
	...
	return r;
	*/
	return Vector{ x + v.x, y + v.y };// 생성자를 이용해서 간단하게도 할 수 있습니다.
}
Vector Vector::operator-(const Vector& v) const
{// V = (Dx-Rx,Dy-Ry)
	return Vector{ x - v.x, y - v.y };// +와 마찬가지로 -로 바꿔 적용해줍니다.
}

Vector Vector::Nomalized()const
{	// 단위 벡터 pi^ = (i - p) / |i - p|
	Vector nomalized = (*this) / Length();	// 자기 자신을 가져와 길이로 나눔. 
	return nomalized;
}
